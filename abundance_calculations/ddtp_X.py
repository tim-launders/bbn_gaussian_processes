# This file computes primordial abundances from sampling from the GP posterior for ddtp S-factor. 
...
from LINX.linx.data.nuclear_rates.key_PArthENoPE import ddtp_frwrd_rate
...
file = './data/ddtp_theory_curve.csv'
...
#hyper = [9.40732505E+01, 2.44754447E+02, 9.25935353E-04, 1.07982256E+03]          # Theory mean
hyper = [4.24454682E+03, 1.38349176E+01, 9.35852483E-04, 1.06890577E+03]          # Zero mean
#hyper = [9.61960754E+04, 2.30694326E+01, 5.90475607E-05, 1.69410532E+04]          # No correlations, zero mean
#hyper = [3.33386014E-02, 3.10473244E+00, 6.54885896E-04, 1.63510612E+03]          # MLE, zero mean
...
num_samples = 10000
num_points = 1000
E_pred = jnp.logspace(np.log10(1e-3), np.log10(1.0), num_points)
...
filepath = './LINX/linx/data/nuclear_rates/key_PRIMAT_2023/ddtp.txt'
...
for sample in samples: 
    ...
    #rate_primat = jnp.array([R(t, E_pred, sample) for t in T_primat])
...
figure_dir = './abundance_calculations/figures/ddtp/'
...
parth_up = np.array([ddtp_frwrd_rate(t, 1) for t in T_plot])
parth_down = np.array([ddtp_frwrd_rate(t, -1) for t in T_plot])
parth_mean = np.array([ddtp_frwrd_rate(t, 0) for t in T_plot])
