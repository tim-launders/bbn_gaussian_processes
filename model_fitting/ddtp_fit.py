...
systematic_error = 0*np.concatenate(all_systematic_error)
statistical_error = np.concatenate(all_statistical_error)
total_error = statistical_error
...
local_kernel = Constant(hyper[2], prior_params=[1e-6,1e6]) * Matern(hyper[3], nu=0.25, prior_params=[1e-6,1e6])
...
E_pred = jnp.logspace(np.log10(1e-3), np.log10(1), num_points)

mean, sigma = gp.predict(jnp.log(E_pred), True)

for i in range(len(points)):
    plt.errorbar(all_E[i], all_S[i], all_total_error[i], fmt=".", color=colors[i], label=sets[i])
...
plt.title(r'$d$($d$,$p$)$t$ Gaussian Process Regression Fit')
