...
class Reaction(eqx.Module): 
    ...
    def __init__(
        ...
        spline_data=None, frwrd_rate_param_func=None, interp_type=None, R_vec=None, T9_vec=None
    ): 
        """
        ...
        R_vec : list, optional
            List of reaction rate as a function of temperature, if a custom rate is desired. 
        T9_vec : list, optional
            List of temperatures, is a custom rate is desired. 
        ...
        if spline_data: 
            ...
            if R_vec is not None:
                if T9_vec is not None:
                    self.T9_vec = T9_vec
                self.mu_median_vec = jnp.array(R_vec)
                self.expsigma_vec = jnp.ones_like(R_vec)
                try:
                    gpus = jax.devices('gpu')
                    self.mu_median_vec = jax.device_put(
