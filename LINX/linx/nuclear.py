...
import LINX.linx.const as const 
from LINX.linx.reactions import Reaction
...
class NuclearRates(eqx.Module): 
    ...
    def __init__(
        ...
        max_i_species=None, reaction_rates = None, rate_arrays=None, 
        temp_arrays=None
    ):
        """
        Populates the class. 
        ...
        reaction_rates : list
            List of reaction indices which which to use custom rates in rate_arrays. 
        rate_arrays : list
            List of arrays of nuclear rates as a function of temperature. 
        temp_arrays : list
            List of arrays of temperatures corresponding with the rates in rate_arrays.
            Not T9, conversion occurs below. 
        """
 
        self.interp_type = interp_type
        self.max_i_species = max_i_species

        # Fill rate_arrays list with none for reactions without custom rates included. 

        all_rates = [None for _ in range(12)]
        all_temps = [None for _ in range(12)]

        if reaction_rates is not None and rate_arrays is not None:

            if len(reaction_rates) == len(rate_arrays):

                arr_ind = 0

                for index in reaction_rates:

                    all_rates[int(index)] = rate_arrays[arr_ind]

                    if temp_arrays is not None:
                        if len(temp_arrays) == len(rate_arrays):
                            all_temps[int(index)] = 1e-9*temp_arrays[arr_ind]
        ...
        if reactions is not None: 
            ...
            self.reactions = self.populate(nuclear_net, rate_arrays=all_rates, temp_arrays=all_temps)
    ...
    def populate(self, network, rate_arrays, temp_arrays): 
        """
        ...
        rate_arrays : list
            List of rate arrays. Entry is none if a custom rate is not desired.
        temp_arrays : list
            List of temperature arrays. Entry is none if custom temperatures are not desired. 
